...
// M5 regression: DbErrorProvider must dedupe 
// identical DbErrorState bursts so downstream 
// consumers don't re-render on every cascade.
let renderCount = 0;
function Probe() {
  useDbError();
  renderCount++;
...
describe("DbErrorProvider — M5 dedupe", () => {
  ...
  it("does not re-render on identical consecutive states", () => {
    render(<Probe />);
    const initial = renderCount;

    act(() => {
      eventBus.emit(EVENT_TYPES.DB_ERROR_CHANGED, { 
        type: "timeout", 
        message: "x" 
      });
    });
