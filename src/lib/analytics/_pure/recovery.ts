...
import type { DisciplineEntry } from "../../planner/types";
...
export function calcRecoveryRate(disciplineLog: DisciplineEntry[]): RecoveryStats | null {
  if (disciplineLog.length < 5) return null;

  const sorted = [...disciplineLog].sort((a, b) => a.date.localeCompare(b.date));
