 ...
 * Content-Type.
 *
 * Invoked via `electron . --verify-headers` from a packaged
 ...
 */
 ...
 // ── C-2 Osiguranje: Striktna pravila za MIME tipove ──
 const EXPECTED_MIME = {
 ...
 function pickDistAssets(distPath) {
   ...
   // Proširena lista: Sada uključuje i vitalne SQLite Worker datoteke
   const fixed = [
     '/sqlite/sqlite3.wasm',
